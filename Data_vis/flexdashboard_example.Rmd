...
library(flexdashboard) # dashboard formatting
...
library(shiny) # interactive features
# setting data
...
renderPlot({
  df %>%
    ...
    ungroup() %>%
    filter(str_detect(sex, if_else(input$v_sex == "All", "ale", input$v_sex))) %>% #filter is used to combine our dropdown select feature with the plots. So now, they update! Note that "ale" is used as it is present in both "Female" and "Male", so it allows both to be displayed.
