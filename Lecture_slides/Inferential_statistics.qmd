...
-   For example, say we were looking to research the effects of exercise on mental health, the null hypothesis be a scenario where exercise would have **zero** effect on mental health.

-   We would then conduct our analysis to examine the extent to which the data aligns/differs from the **zero effect**.
:::
...
## Core components of Null Hypothesis Tests {.scrollable}

::: incremental
-   The **alpha cut-off**; as researchers we must make an informed decisions of the minimum **p-value** we want to observe to reject the null hypothesis.
...
-   In psychology we tend to use .05 as the cut off. In clinical research where human lives are implicated, we might use a stricter .01 alpha.

-   Fun/scary fact: the reason why .05 is commonly used, is because Fischer suggested "1 in 20 should do the trick"\*.

-   `*`Somewhat paraphrased, but it was an off the cuff remark that started our craze with *p* \< .05.
:::
...
::: incremental
-   In this case, the data was collected pre and post intervention.

-   The outcome measure used was peak airflow (a commonly used medical test to help determine severity of asthma).
...
-   For the null hypothesis (*H0*), we expect **zero** change in peak airflow for asthma patients who use the ***New Drug***.

-   For the alternative hypothesis (*H1*), we expect the use of the ***New Drug*** to **significantly improve** peak airflow for asthma patients.
:::
...
-   We report this along with our descriptive statistics: At post intervention, peak airflow (*Mean* = 482.39, *sd* = 72.13) significantly increased (*t* = 4.49, *df* = 19, *p* = .0003) in comparison to pre intervention peak airflow (*Mean* = 409.60, *sd* = 65.63).
...
# As our alpha is .01, we need a .995 Confidence interval
ci <- qt(0.995, df = nrow(New_Drug) - 1) * se_diff
...
# Making the plot
## Note: I use 2 datasets here. One for presenting the error bars (made above), and the original dataset to present the distribution of the data in the violin plot.

ggplot(data = plot_data, # Main data of plot on confidence intervals made above
       aes(x = Time_Point, y = mean, fill = Time_Point)
       ) +
  geom_half_violin(data = New_Drug, # Original data for violin plot
       aes(x = Time_Point, y = Peak_flow),
       alpha = .4)+
  geom_errorbar(data = plot_data,
       aes(ymin = lower_ci, ymax = upper_ci, color = Time_Point), #setting the height and color of error bar based on our confidence intervals and groups.
       width = 1, size = 1) +
  stat_summary(data = plot_data,
       geom = "point", fun = "mean", size = 3, #creating a coloured mean value to the error bars.
...
-   However, we do have some statistical tricks to help us. Many inferential statistics can be made into [***standardised effect sizes***](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3840331/).
...
# As our alpha is .01, we need a .995 Confidence interval
ci <- qt(0.995, df = nrow(Placebo) - 1) * se_diff
lower_ci <- mean_diff - ci
upper_ci <- mean_diff + ci
...
ind_t_test_data %>% 
  group_by(Treatment) %>% 
  summarise(n = n(),
            Mean_Peak_flow_difference = mean(Peak_flow_difference),
            SD_Peak_flow_difference  = sd(Peak_flow_difference)
            ) %>%
  gt() %>% fmt_number(decimals = 3)
```
...
## The independent samples t-test {.scrollable}
...
-   This is very similar to interpreting the paired samples t-test results. Only this time, we are comparing between the groups as opposed to comparing between time points.

-   As this data still involves clinical trials, we are still using the alpha cutoff of .01.

-   The analysis shows a signficant difference between the placebo and new drug treatment (*t*(38.00) = 2.78, *p* =.008), displaying a large effect size (*D* = .88).

-   The change is peak flow of the ***new drug*** treatment (*M* = 72.79, *sd* = 73.30) was greater than the change of peak flow within the ***placebo*** treatment (*M* = 8.19 , *sd* = 73.79).

## The independent samples t-test {.scrollable}

### Interpreting the analysis (Data vis)
...
## Critically evaluating research {.scrollable}
...
-   The grumpy researcher in the room insists that we collect additional data on the current recommended treatment for asthma; the Ventolin inhaler.

-   Why would they argue for this?
...
-   It is [recommended for research](https://academic.oup.com/beheco/article/19/3/690/186443) as it brings a good balance between statistical power and preventing ***type 1*** errors.
:::
...
-   No significant difference (*p* = .113) was reported between Ventolin (*M* = 117.27, *sd* = 59.21) and the New Drug (*M* = 72.79, *sd* = 73.30).
:::
:::::
...
## Addressing the research question {.scrollable}
...
-   The **new drug** ***significantly improves*** the change in airflow in comparison to the placebo in the initial ***t-test*** analyses.
...
-   There is ***no significant difference*** between **ventolin** and the **new drug**.

-   With the Tukey correction, there is ***no significant difference*** between the **New drug** and **Placebo** treatment at the **alpha of .01**.
:::
...
-   <https://quantitudepod.org/> : Amazing nerdy podcast on a variety of statistical topics. Use their search bar, and dive into the discussion between the quantitative clinical/social science researchers.
...
-   
    ...
    ```         
    [Lakens, D. (2013). Calculating and reporting effect sizes to facilitate cumulative science: a practical primer for t-tests and ANOVAs. Frontiers in psychology, 4, 863.](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3840331/).
    ```
...
-   [Ruxton, G. D., & Beauchamp, G. (2008). Time for some a priori thinking about post hoc testing. Behavioral ecology, 19(3), 690-693.](https://academic.oup.com/beheco/article/19/3/690/186443).
...
-   [Tidy Data. Journal of Statistical Software 59 (10). Wickham, Hadley, and Garrett Grolemund. 2016. R for Data Science: Import, Tidy, Transform, Visualize, and Model Data. O’Reilly Media, Inc.](https://vita.had.co.nz/papers/tidy-data.pdf) - Nerdy but useful paper on data tidying in R; what tidy data is, how we achieve it, and how it makes our life as researchers easier.
